...
The registry is stored under `~/.craft-agent/runtime/recovery-incidents/`. Stable keys deduplicate repeated observations. Incidents move through `open`, `claimed`, `deferred`, `resolved`, `escalated`, or `suppressed`. Claims and controller ownership use atomic file locks and expiry leases. Expired claim/controller owners cannot heartbeat or mutate. The kill switch blocks claims, heartbeats, and incident mutations; `controller-release` remains the sole allowed fail-safe mutation so a disabled controller can relinquish its lock. Coordinator claims expose deterministic `wake-1`, `wake-2`, then `rotation` stages with stage-specific allowed actions.
