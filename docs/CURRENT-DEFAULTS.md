...
Correction extension (3.4.26): one self-granted bounded extension per story; the second → correction-budget-extension-reused
Gate scope (3.4.26):           gates only for external effects; investigation/root-cause/audit is self-authorized
Complete idle (3.4.26):        complete standing 1800s with no next action or gate → complete-without-next-increment (CRAFT_STATUS_COMPLETE_IDLE_SECONDS)
Vanished coordinator (3.4.26): verify-session-absent then respawn-from-handoff-snapshot before owner escalation
Orphaned lane (3.4.26):        dead lane >86400s whose dispatcher owns no project → orphaned-dead-lane (CRAFT_ORPHANED_LANE_SECONDS)
Increment cards (3.4.26):      one subtask per story; status follows story state; --reset-cards rebuilds cards + subtasks
Kill switch re-arm (3.4.26):   install.sh restores the pre-install self-healing state after tests pass
Kill switch visibility (3.4.26): detect/report expose killSwitch.staleWithOpenConditions past 1800s (CRAFT_KILL_SWITCH_STALE_SECONDS)
Story acceptance (3.4.26):     accepted story needs observed acceptanceRef + workUnit → story-accepted-without-evidence / -ref-not-observed
Delivery verification (3.4.26): story mergeSha must be ancestor of origin/<origin/HEAD branch>; merge commit only, never candidate
Delivered stages (3.4.26):     deploying/demonstrating/complete require a merge commit per accepted story → unmerged-delivery
Pull request duty (3.4.26):    actionable PR parked >3600s → pull-request-unfinished; check older than 21600s → pull-request-check-stale
Lease registration (3.4.26):   lease-less child of a live coordinator >600s → unregistered-child-lane (CRAFT_UNREGISTERED_CHILD_SECONDS)
Standing merge authority (3.4.26): per project + exact branch, direct-owner only, risk ceiling, TTL default 604800s (max CRAFT_STANDING_AUTHORITY_MAX_TTL_SECONDS 2592000)
Standing merge refusals (3.4.26): invalid certificate, candidate absent or already in branch, unauthorised branch, risk above ceiling, project HOLD or work-unit gate
Standing merge receipt (3.4.26): written by `use` before the merge; absence next to a delivered story is detectable
Named merge authority (3.4.26): story mergeAuthorityRef must be a resolved gate id or a standing-merge work unit → merge-without-named-authority
Blocked story binding (3.4.26): blocked story with satisfied deps needs blockedByRef naming an open gate/wait/blockerRef
Handoff deadline (3.4.26):     handoff-ready lane older than 600s → handoff-unconsumed (CRAFT_STATUS_HANDOFF_GRACE_SECONDS)
Merge publication (3.4.26):    standing-merge receipt without a declared mergeSha → merge-receipt-unpublished
Idle-ready scope (3.4.26):     a story with a handoff-ready lane is not idle; consumption has its own deadline
Ledger drain order (3.4.26): safety, pipeline blockers, lane recovery, housekeeping (quota CRAFT_DRAIN_HOUSEKEEPING_QUOTA=1, limit CRAFT_DRAIN_LIMIT=3)
Controller scope (3.4.26):     a valid lease works the open backlog; the admission envelope is a wake reason, not the work list
Plan executors (3.4.26):       nextActions[].executor required in flight-free turns; owner-gate actions need an open gateRef
Preparation duty (3.4.26):     gate-blocked idle with no agent-executed action → idle-without-preparation
Probe deferral (3.4.26):       unobservable safety fact defers the cycle; durable block after 3 consecutive failures (CRAFT_ADMISSION_MAX_PROBE_FAILURES)
Detection independence (3.4.26): the admission cron runs detect --apply every tick; perception never waits for a controller turn
Controller silence (3.4.26):   blocking backlog + no controller turn within 1800s + no kill switch → drain reports controller.silent (CRAFT_CONTROLLER_SILENT_SECONDS)
Transport health (3.4.26):     admission tick records channel success/failure; drain reports transport.lost after 900s without success (CRAFT_TRANSPORT_LOST_SECONDS)
...
GitHub sync (3.4.26):          githubSync {issue, commentRef, projectField, syncedStage, syncedAt}; material stage without it → github-sync-missing/stale
