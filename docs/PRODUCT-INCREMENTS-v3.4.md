...
### v3.4.40 exact completion consumption

For a v3.4.40 coordinator, immutable completion bindings are necessary but not sufficient: the integrated handoff supplies exactly one `candidateSha:<40-hex>` marker; acceptance repeats that exact candidate and `verdict:PASS`; the observer readback repeats the candidate and every delivered `merged-main:<mergeSha>`. The existing event key/revision/fingerprint tuple prevents revision substitution, while these markers prevent semantic substitution. A mismatch fails publication. Legacy v3.4.36 records remain readable.
