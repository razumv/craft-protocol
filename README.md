...
- `release-closure.py verify` is read-only and refuses closure without authenticated `gh` API identity, annotated tag peeling exactly to `origin/main`, exact Release/Latest/freshness/assets, fleet adoption record, manifest, and no-mutation installer proof. It never tags, pushes, installs, or calls unauthenticated release data.
