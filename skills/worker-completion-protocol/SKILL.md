...
# Worker Completion Protocol v3.4.37
...
downgrade a terminal/blocker report with a later progress report. For v3.4.37 completion, an integrated candidate handoff must include one exact `candidateSha:<40-hex>` marker; an auditor PASS repeats that exact marker with `verdict:PASS`, and a readback watcher repeats it with its exact `merged-main:<mergeSha>` marker. These are machine-consumed bindings, not a narrative.
