...
class AdmissionV3435(unittest.TestCase):
 def setUp(self):
  ...
  self.manifest('coord','coordinator'); self.manifest('worker','worker',['parent-session::coord','work-unit::u','attempt::1'])
 def tearDown(self):self.t.cleanup()
 def manifest(self,sid,role,labels=()):
  wt=self.r/f'wt-{sid}';wt.mkdir(parents=True,exist_ok=True);p=self.sessions/sid;p.mkdir(parents=True,exist_ok=True)
  (p/'session.jsonl').write_text(json.dumps({'id':sid,'isArchived':False,'sessionStatus':'todo','workspaceRootPath':str(self.r),'workingDirectory':str(wt),'labels':[f'agent-role::{role}',*labels]})+'\n')
