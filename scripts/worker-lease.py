...
def validate_existing_admission(session_id: str) -> None:
    ...
    if not ({"protocol-version::3.4.35", "protocol-version::3.4.36", "protocol-version::3.4.37"} & set(manifest.get("labels") or [])): return
    ...
    if len(matches) != 1: raise SystemExit("v3.4.35/v3.4.36/v3.4.37 lane has no unique admitted record")
...
def parser() -> argparse.ArgumentParser:
    ...
    c.add_argument("--admission-token", help="confirmed v3.4.35/v3.4.36/v3.4.37 lane-admission token")
