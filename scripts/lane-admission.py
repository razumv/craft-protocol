...
"""Strict two-phase admission for v3.4.35/v3.4.36/v3.4.37/v3.4.38 worker/auditor lanes; legacy records pass unchanged."""
...
ROOT=common.RUNTIME/'lane-admissions'; LOCK=common.RUNTIME/'lane-admissions.lock'; ROLES={'worker','auditor'}; VERSIONS={'3.4.35','3.4.36','3.4.37','3.4.38'}
def clean(x):
...
def collisions(identity,session=None):
 ...
 # A transferred child can have no lease at all.  Its live/needs-review manifest
 # is still a concrete executor, so a new same-role/work-unit reservation would
 # duplicate work after a rotation merely because runtime bookkeeping was stale.
 registry=common.read_json(common.RUNTIME/'coordinators'/f"{identity['project']}.json") or {}
 parents={identity['parentSessionId']}
 if registry.get('coordinatorSessionId')==identity['parentSessionId'] and registry.get('predecessorSessionId'): parents.add(str(registry['predecessorSessionId']))
 for sid,m in common.all_manifests().items():
  if sid==session or not common.session_live(m) or role(m)!=identity['role']: continue
  parent=lab(m,'parent-session::') or m.get('parentSessionId')
  if parent in parents and lab(m,'work-unit::')==identity['workUnit']:
   raise SystemExit('live manifest work-unit collision')
def cmd_reserve(a):
...
def cmd_confirm(a):
 with common.file_lock(LOCK):
  ...
  if not strict(m) or not required.issubset(set(m.get('labels') or [])): raise SystemExit('v3.4.35/v3.4.36/v3.4.37/v3.4.38 canonical lane labels required')
  if m.get('llmConnection')!='chatgpt-plus' or m.get('model')!='pi/gpt-5.6-terra' or m.get('permissionMode') not in {'allow-all','execute'}: raise SystemExit('v3.4.35/v3.4.36/v3.4.37/v3.4.38 worker/auditor runtime identity mismatch')
