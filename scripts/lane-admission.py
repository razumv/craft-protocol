...
"""Strict two-phase admission for v3.4.35/v3.4.36/v3.4.37 worker/auditor lanes; legacy records pass unchanged."""
...
ROOT=common.RUNTIME/'lane-admissions'; LOCK=common.RUNTIME/'lane-admissions.lock'; ROLES={'worker','auditor'}; VERSIONS={'3.4.35','3.4.36','3.4.37'}
def clean(x):
...
def cmd_confirm(a):
 with common.file_lock(LOCK):
  ...
  if not strict(m) or not required.issubset(set(m.get('labels') or [])): raise SystemExit('v3.4.35/v3.4.36/v3.4.37 canonical lane labels required')
  if m.get('llmConnection')!='chatgpt-plus' or m.get('model')!='pi/gpt-5.6-terra' or m.get('permissionMode') not in {'allow-all','execute'}: raise SystemExit('v3.4.35/v3.4.36/v3.4.37 worker/auditor runtime identity mismatch')
